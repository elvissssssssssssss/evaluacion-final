...
def nodo_1_simple(estado: Estado) -> Estado:
    respuesta = invoke_llm("llama3.2", "Responde a este mensaje en español: " + estado["mensaje"], streaming=True)
    return {"mensaje": respuesta}

def nodo_2_simple(estado: Estado) -> Estado:
    respuesta = invoke_llm("llama3.2", "Resume este texto en español: " + estado["mensaje"], streaming=True)
    return {"mensaje": respuesta}

def nodo_3_simple(estado: Estado) -> Estado:
    respuesta = invoke_llm("llama3.2", "Analiza el sentimiento de este texto en español (positivo, negativo, neutral): " + estado["mensaje"], streaming=True)
